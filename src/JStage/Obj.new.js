...
JStage.Obj.INIT_STATE = 'INIT_STATE'; // 初始化状态
JStage.Obj.OPEN_STATE = 'OPEN_STATE'; // 开场状态
...
JStage.Obj.prototype = {
     ...
     * @param {string|int} delay 
     * @param {string} timingFunction 
     * @returns {this}
     */
    addState: function(stateName, state, duration, delay, timingFunction) {
        if (undefined === this.states[stateName]) {
            this.states[stateName] = {};
        }

        this.states[stateName] = this.createState(state, duration, delay, timingFunction);
     ...
     * @returns {object}
     */
    createState: function(state, duration, delay, timingFunction) {
        return {
            state: state,
            duration: JStage.normalizeTime(duration),
            delay: JStage.normalizeTime(delay),
            timingFunction: undefined === timingFunction ? 'linear' : timingFunction,
        }
    },
    ...
    getState: function(offset) {
        if (undefined === this.states[offset]) {
            throw 'Unknown state ' + offset;
        }
     ...
     */
    getStateDiff: function(fromState, toState) {
        // var diffState = this.createState({}, toState.duration, toState.delay, toState.timingFunction);

        // for (var prop in toState.state) {
        //     diffState.state[prop] = this.getPropDiff(
        //         prop,
        //         (undefined === fromState.state[prop] ? this.getPropDefaultValue(prop) : fromState.state[prop]), // prop from,
        //         toState.state[prop]
        //     );
        // }
     ...
     */
    updateState: function(fromState, diffState, progress) {
        for (var prop in diffState) {
            this.intermediateState[prop] = progress * this.stage.scale;
        }
    },

    getPropDiff: function(prop, from, to) {
        if (prop.indexOf('color')) {
